...
(function() {
  var addCouple, addDance, addJudge, calculateCoupleMarks, calculateInter, calculateJudgeMarks, calculateMaxSums, couples, dances, generateCoupleMarks, generateInter, generateJudgeMarks, generateMarks, judges, marks, pastein, pasteinDanceresults, pasteinO2cm, removeCouple, removeDance, removeJudge, sortCoupleMarks, sortJudgeMarks, test_addCouple, test_addDance, test_addJudge, test_addMarks, validateMarks;
  ...
  $(function() {
    ...
    $('#judgeMarksGen').click(function() {
      return generateJudgeMarks();
    });
    $('#coupleMarksGen').click(function() {
      return generateCoupleMarks();
    });
    return $('#pasteinButton').click(function() {
      return pastein();
    });
  });
  ...
  test_addCouple = function(number, name) {
    var idx;
    idx = couples.length;
    addCouple();
  ...
  test_addJudge = function(number, name) {
    var idx;
    idx = judges.length;
    addJudge();
  ...
  test_addDance = function(number, name) {
    var idx;
    idx = dances.length;
    addDance();
  ...
  test_addMarks = function(newMarks) {
    var c, counter, d, j, markList, numCouples, numDances, numJudges, _i, _j, _k, _ref, _ref1, _ref2;
    generateMarks();
    numDances = newMarks.length;
    numCouples = newMarks[0].length;
    numJudges = newMarks[0][0].length;
    markList = [];
    for (d = _i = 0, _ref = numDances - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; d = 0 <= _ref ? ++_i : --_i) {
      for (c = _j = 0, _ref1 = numCouples - 1; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; c = 0 <= _ref1 ? ++_j : --_j) {
        for (j = _k = 0, _ref2 = numJudges - 1; 0 <= _ref2 ? _k <= _ref2 : _k >= _ref2; j = 0 <= _ref2 ? ++_k : --_k) {
    ...
    counter = 0;
    return $('#marks input').each(function(index) {
      return $(this).val(markList[counter++]);
    });
  };
  ...
  calculateCoupleMarks = function() {
    var c, coupleMarks, d, j, markSum, numCouples, numDances, numJudges, _i, _j, _k, _ref, _ref1, _ref2;
    coupleMarks = [];
    numDances = marks.length;
    numCouples = marks[0].length;
    numJudges = marks[0][0].length;
    for (c = _i = 0, _ref = numCouples - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; c = 0 <= _ref ? ++_i : --_i) {
      coupleMarks[c] = [];
      for (j = _j = 0, _ref1 = numJudges - 1; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; j = 0 <= _ref1 ? ++_j : --_j) {
        markSum = 0;
        for (d = _k = 0, _ref2 = numDances - 1; 0 <= _ref2 ? _k <= _ref2 : _k >= _ref2; d = 0 <= _ref2 ? ++_k : --_k) {
          markSum += parseInt(marks[d][c][j]);
        }
  ...
  sortCoupleMarks = function(coupleMarks) {
    var coupleMark, idx, sortedMarks, _i, _len;
    sortedMarks = [];
    for (idx = _i = 0, _len = coupleMarks.length; _i < _len; idx = ++_i) {
      coupleMark = coupleMarks[idx];
      sortedMarks.push({
  ...
  generateCoupleMarks = function() {
    var couple, coupleHtml, coupleIdx, coupleMarks, judge, sortedJudges, _i, _j, _len, _len1, _results;
    if (!validateMarks()) {
      return;
    }
    ...
    $('#coupleMarks').empty();
    _results = [];
    for (coupleIdx = _i = 0, _len = coupleMarks.length; _i < _len; coupleIdx = ++_i) {
      couple = coupleMarks[coupleIdx];
      coupleHtml = "<table><tr><th></th><th>" + couples[coupleIdx].name + "</th></tr>";
      sortedJudges = sortCoupleMarks(couple);
      for (_j = 0, _len1 = sortedJudges.length; _j < _len1; _j++) {
        judge = sortedJudges[_j];
        coupleHtml += "<tr><td>" + judge.name + "</td><td>" + judge.mark + "</td></tr>";
      }
      coupleHtml += "</table>";
      _results.push($('#coupleMarks').append(coupleHtml));
    }
    return _results;
  };

  pastein = function() {
    var pageType, pasteStr;
    pasteStr = $('#pastein').val();
    pageType = $('#pasteinSelect').val();
    switch (pageType) {
      case "o2cm":
        return pasteinO2cm(pasteStr);
      case "danceresults":
        return pasteinDanceresults(pasteStr);
    }
  };

  pasteinDanceresults = function(pasteStr) {
    var coupleIdx, danceCode, danceIdx, endText, gotDance, judgeArr, judgeIdx, judgePos, judgeRe, lastJudgePos, line, linesToSkip, markArr, markRe, newMarks, splitLine, tmp, totalCouples, totalJudges, _i, _j, _k, _l, _len, _len1, _len2;
    judgeRe = /(Judge Name(?:.|\n)*)/g;
    ...
    for (_k = 0, _len2 = tmp.length; _k < _len2; _k++) {
      ...
      if (!gotDance) {
        ...
        if (line === "") {
          ...
          for (judgePos = _l = 3; 3 <= lastJudgePos ? _l <= lastJudgePos : _l >= lastJudgePos; judgePos = 3 <= lastJudgePos ? ++_l : --_l) {
            judgeIdx = judgePos - 3;
    ...
    return test_addMarks(newMarks);
  };

  pasteinO2cm = function(pasteStr) {
    var coupleArr, coupleCounter, coupleIdx, coupleRe, coupleToIndex, danceCode, danceIdx, gotDance, judgeIdx, judgePos, line, linesToSkip, newMarks, number, parsingCouples, splitLine, tmp, totalCouples, totalJudges, _i, _j, _k, _len, _len1;
    coupleRe = /(Couples(?:.|\n)*)/g;
